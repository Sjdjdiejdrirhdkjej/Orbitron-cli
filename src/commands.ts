...
export function buildStatusReport(): string {
  ...
  const health =
    store.backendHealth === "ok"
      ...
      ? "error"
      : "checking";
  const model = getDisplayName(store.config.model);
...
function buildHelpReport(): string {
...
export function getCommands(): Command[] {
  ...
  return [
    ...
    },
    {
      ...
      aliases: ["/export", "/save"],
      execute: () => {
        const { messages, config } = useChatStore.getState();
        if (messages.length === 0) {
          useChatStore.getState().addMessage({ role: "assistant", content: "No conversation to export." });
          return;
        }
        const lines: string[] = [];
        ...
        try {
          ...
          useChatStore.getState().addMessage({ role: "assistant", content: `Exported "${title}" to clipboard (markdown format). Paste with Ctrl+V.` });
        } catch {
          useChatStore.getState().addMessage({ role: "assistant", content: "Export failed — could not write to clipboard." });
        }
      },
    },
    {
      ...
      execute: () => {
        ...
        useChatStore.getState().addMessage({ role: "assistant", content: "No code block found in recent messages." });
      },
    },
    {
      name: "search",
...
function aliasMatchesInput(alias: string, trimmedInput: string): boolean {
  const lowerAlias = alias.toLowerCase();
  if (trimmedInput.toLowerCase() === lowerAlias) return true;
  if (!trimmedInput.toLowerCase().startsWith(lowerAlias)) return false;
  const nextChar = trimmedInput.slice(lowerAlias.length, lowerAlias.length + 1);
  return /\s/.test(nextChar);
}

export function matchCommandWithArgs(input: string): { command: Command; arg: string } | null {
