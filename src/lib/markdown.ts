...
export interface MarkdownSegment {
  type: "text" | "bold" | "italic" | "code" | "codeBlock" | "highlightedCode" | "link" | "list" | "blockquote" | "h1" | "h2" | "h3" | "hr" | "latex" | "latexBlock" | "table";
  content: string;
  lang?: string;
  headers?: string[];
  rows?: string[][];
...
const CODE_LANG_COLORS: Record<string, string> = {
...
// Syntax tokenizer
type TokenType = "keyword" | "string" | "comment" | "number" | "operator" | "punctuation" | "function" | "type" | "plain";
...
type Lang = "js" | "ts" | "python" | "bash" | "json" | "html" | "css" | "sql" | "yaml" | "go" | "rust" | "plain";

interface RawToken { type: TokenType; value: string }

function tokenize(code: string, lang: Lang): RawToken[] {
  ...
  while (i < src.length) {
    ...
    if (/[0-9]/.test(src[i]) && (i === 0 || !/[a-zA-Z_]/.test(src[i - 1]))) {
      let j = i;
      while (j < src.length && /[0-9.xXa-fA-FuUlL_]/.test(src[j])) j++;
    ...
    if (/[a-zA-Z_$]/.test(src[i])) {
      ...
      else if (lang === "python" && PY_KW.has(word)) type = "keyword";
      else if (TYPES.has(word) || (word[0] === word[0].toUpperCase() && word[0] !== word[0].toLowerCase())) type = "type";
      else if (BUILTINS.has(word) && isFn) type = "function";
      else if (isFn) type = "function";
...
function langFromString(lang: string): Lang {
  const l = lang.toLowerCase();
  if (/^(js|javascript)$/.test(l)) return "js";
  if (/^(ts|typescript)$/.test(l)) return "ts";
  if (/^python$/i.test(l) || l === "py") return "python";
  if (/^bash|shell|sh$/.test(l)) return "bash";
  if (/^go|golang$/i.test(l)) return "go";
  if (/^rust$/i.test(l)) return "rust";
  if (/^sql$/i.test(l)) return "sql";
  if (/^yaml$/i.test(l)) return "yaml";
  if (/^json$/i.test(l)) return "json";
  if (/^html$/i.test(l)) return "html";
...
// Markdown parser
export function parseMarkdown(raw: string): MarkdownSegment[] {
  ...
  for (const line of lines) {
    ...
    if (cleaned.trim()) cleanedLines.push(cleaned);
    else if (line.trim() === "") cleanedLines.push(line);
  }

  let i = 0;

  const flushText = (buf: string): void => {
    if (!buf) return;
  ...
  while (i < cleanedLines.length) {
    ...
    if (/^\$\$/.test(line)) {
      ...
      if (closingLine) latexLines.push(closingLine);
      i++;
      while (i < cleanedLines.length && !/^\$\$/.test(cleanedLines[i])) {
...
function parseTable(lines: string[]): TableParseResult | null {
  ...
  if (lines.length >= 2) {
    ...
    for (const cell of sepCells) {
      ...
      if (/^:-+:$/.test(trimmed)) alignments.push("center");
      else if (/^-+:$/.test(trimmed)) alignments.push("right");
      else if (/^:-+$/.test(trimmed)) alignments.push("left");
...
function splitInline(text: string): string[] {
  ...
  while (i < text.length) {
    if (text[i] === "`" && text[i + 1] === "`" && text[i + 2] === "`") {
      ...
      let j = i + 3;
      while (j < text.length && !(text[j] === "`" && text[j + 1] === "`" && text[j + 2] === "`")) j++;
...
export function getSegmentFg(seg: MarkdownSegment, theme: Theme, selected: boolean): string {
  switch (seg.type) {
...
export function getSegmentAttrs(seg: MarkdownSegment): number {
  if (seg.type === "bold" || seg.type === "h1" || seg.type === "h2" || seg.type === "h3") return 1;
  if (seg.type === "italic") return 2;
  if (seg.type === "codeBlock" || seg.type === "highlightedCode") return 1;
  return 0;
}

export function getLangColor(lang: string | undefined, theme: Theme): string {
...
export function getTokenFg(type: TokenType): string {
  return TOKEN_COLORS[type] ?? TOKEN_COLORS.plain;
}

// LaTeX math rendering (display and inline)
function parseLatexBlock(src: string, startIdx: number): { content: string; isBlock: boolean; endIdx: number } | null {
  if (src[startIdx] !== "$") return null;
  const isBlock = src[startIdx + 1] === "$";
  if (isBlock && src[startIdx + 2] === "$") return null; // skip $$$
...
// Simple LaTeX tokenizer for syntax colouring
type LatexTokenType = "command" | "argument" | "group" | "symbol" | "number" | "text";

interface LatexToken { type: LatexTokenType; value: string }
...
export function renderLatex(latex: string, isBlock: boolean): { tokens: LatexToken[]; greek: Map<string, string> } {
...
export type { LatexToken, LatexTokenType };

export const LATEX_TOKEN_COLORS: Record<LatexTokenType, string> = {
...
export function getLatexTokenFg(type: LatexTokenType): string {
  return LATEX_TOKEN_COLORS[type] ?? "#d4d4d4";
}

export function getLatexBg(isBlock: boolean): string {
  return isBlock ? "#1a1a2e" : "transparent";
}

export function getLatexFg(isBlock: boolean): string {
